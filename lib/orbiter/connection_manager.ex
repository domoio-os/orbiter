defmodule Orbiter.ConnectionManager do
  ...
  defp start_connection(state) do
    case Orbiter.Config.get(:device_id) do
      nil -> state
      device_id -> connect(state)
    end
  end
  ...
  def handle_call(:get_state, _from, state) do
    ...
    configured = Orbiter.Config.get(:device_id) != nil
    {:reply, %{connected: connected, configured: configured, version: Orbiter.version}, state}
  end
